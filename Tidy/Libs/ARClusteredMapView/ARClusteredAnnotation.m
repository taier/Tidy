...
- (NSString *)title {
	if (0 < [self.containedAnnotations count]) {
		...
		NSString *title = [NSString stringWithFormat:@"%lu %@", [self.containedAnnotations count]+1, localizedAnnotations];
		return title;
	}
	return _title;
}
...
- (NSString *)description {
	return [NSString stringWithFormat:@"<%@: %p> title = %@; subtitle = %@; cluster = %@; # contained: %lu; coordinate: %@",
			NSStringFromClass([self class]),
			self,
			self.title,
			self.subtitle,
			self.clusterAnnotation ? [NSString stringWithFormat:@"\n%@\n", self.clusterAnnotation] : @"NO",
			(unsigned long)[self.containedAnnotations count],
